...
class Predictor():
    ...
    def __init__(self,
        ...
        self.data_present_path="./data/conll_2003/present.json"
    ...
    def build_chat_input(self, query, history=None):
        ...
        input_sequence = content.split(" 输入: ")[1].strip().split(' ') # 用空格分割得到列表
        # 这里对于单条句子会得到二维的列表，例如[[20565], [101961], [21], [103319], [13, 99698, 24], [98360], [16, 14, 17]]
        input_sequence = [
            self.tokenizer.encode(char, add_special_tokens=False)  # 对于中文一般不会分词，所以之前用[0]，英文不行
    ...
    def predict(self, query: str | list = '', history: List = None, num_samples=5, max_length=512, max_new_tokens=512, num_beams:int=1, top_p: float = 0.8, temperature=1.0, do_sample: bool = False, build_message=False, batch_size=2):
        test_path = self.data_present[self.data_path]['dev']
        ...
        with torch.no_grad():
            ...
            for sample_id in range(num_samples):
                    ...
                    for seq_idx in range(max_char_num):
                        ...
                        for b in range(batch_size):
                            if seq_idx < char_num_list[b]:
                                # 当前字符 token id
                                char_token = input_sequence[b][seq_idx]
                                # 更新 input_idx[b]（全部在 CPU 上，是一个 list），直接 extend 两个 list
                                input_idx[b].extend(char_token)
                        ...
                        for step in range(self.label_token_len):
                            # 仅处理活跃样本
                            # 对于中文情况，前面已经对inout_idx进行对齐了，且每次对于活跃的样本都只添加一个token，所以始终保持对齐
                            # 对于英文情况，得再次对活跃样本进行对齐（因为每个样本添加的token个数会不一样）
                            ...
                            for idx, orig_batch_idx in enumerate(active_indices):
                                ...
                                for label, token_seq in self.candidates.items():
                                    ...
                                    cand_probs.append(probs[idx, candidate_token].item())

                                # 概率归一化
                                cand_probs_tensor = torch.tensor(cand_probs, device=self.device)
                                cand_probs_tensor = cand_probs_tensor.pow(1/temperature)    # 对概率做温度缩放，抑制高概率极端值，提升小概率 token 的采样几率
                                cand_probs_tensor = cand_probs_tensor / cand_probs_tensor.sum()
                                # 多项式采样
                                gen = torch.Generator(device=self.device).manual_seed(torch.randint(0,2**31,[1]).item())  # 随机数
                                id = torch.multinomial(cand_probs_tensor, num_samples=1, generator=gen).item()
                                chosen_token = cand_tokens[id]
                                # 追加到当前 sample 的 input_idxs
                                input_idx[orig_batch_idx].append(chosen_token)
                ...
                # 指定目标目录
                out_dir = "./data_record/predict_llama_conll2003_1000_nt2_1_75000_dev_sample7"
                # 递归创建目录，若已存在则不报错
                os.makedirs(out_dir, exist_ok=True)

                with open(f"{out_dir}/output_nt2_sample{sample_id}.txt", "w", encoding="utf-8") as file:
                    for s in preds2:
                        file.write(s + "\n")  # 每个字符串末尾添加换行符
    ...
    def __call__(self, query: str | list = '', history: List = None, num_samples=10, max_length=512, max_new_tokens=512, num_beams:int=1, top_p: float = 0.8, temperature=5.0, do_sample: bool = False, build_message=True, batch_size = 2):
        self.predict(query, history, num_samples, max_length, max_new_tokens, num_beams, top_p, temperature, do_sample, build_message, batch_size)
