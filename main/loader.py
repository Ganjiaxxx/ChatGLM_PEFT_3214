...
from main.loaders.llm_chat_en import LLMChatENDataset
from main.loaders.chatglm_rlhf import ChatGLM_RLHFDataset
from main.loaders.llm_dpo import LLMDPODataset
from main.loaders.llm_dpo_en import LLMDPOENDataset
import torch
import numpy as np
...
def collate_fn_wrapper_dpo(tokenizer):
    def left_pad_collate_fn(batch):
        ...
        max_length = 0
        for key in ["chosen", "rejected", "input_ids"]:
        ...
        for item in batch:
            for key in item:
                ...
                elif key in ("chosen_mask", "rejected_mask"):
                    pad_length = max_length - len(item[key])
                    # 之前chosen_mask和rejected_mask是到补齐的这步才开始生成（如上面的代码），并且mask掉prompt和pad部分
                    # 现在chosen_mask和rejected_mask一早就生成，现在只需要用0补齐
                    item[key] = torch.cat([torch.LongTensor([0] * pad_length), item[key]], dim=-1)
                    result[key].append(item[key])
                elif key == ("input_ids"):
                    pad_length = max_length - len(item[key])
                    item[key] = torch.cat([torch.LongTensor([tokenizer.pad_token_id] * pad_length), item[key]], dim=-1)
...
class AutoDataloader():
    ...
    def __init__(self, tokenizer, config, loader_name='LLM_Chat', data_path="Boss", data_present_path="./data/mit_movies/present.json", max_length=50):
        ...
        if loader_name == 'LLM_Pure':
            ...
            if 'test' in self.data_path:
                self.test_set = LLMChatDataset(
                    tokenizer, config, self.data_path['test'], max_length=self.max_length, do_shuffle=False)
        elif loader_name == 'LLM_Chat_EN':
            self.train_set = LLMChatENDataset(
                tokenizer, config, self.data_path['train'], max_length=self.max_length, do_shuffle=True)
            self.eval_set = LLMChatENDataset(
                tokenizer, config, self.data_path['dev'], max_length=self.max_length, do_shuffle=False)
            if 'test' in self.data_path:
                self.test_set = LLMChatENDataset(
                    tokenizer, config, self.data_path['test'], max_length=self.max_length, do_shuffle=False)
            ...
            if 'test' in self.data_path:
                self.test_set = LLMDPODataset(
                    tokenizer, config, self.data_path['test'], max_length=self.max_length, do_shuffle=False)
        elif loader_name == 'LLM_DPO_EN':
            self.train_set = LLMDPOENDataset(
                tokenizer, config, self.data_path['train'], max_length=self.max_length, do_shuffle=True)
            self.eval_set = LLMDPOENDataset(
                tokenizer, config, self.data_path['dev'], max_length=self.max_length, do_shuffle=False)
            if 'test' in self.data_path:
                self.test_set = LLMDPOENDataset(
                    tokenizer, config, self.data_path['test'], max_length=self.max_length, do_shuffle=False)
    ...
    def __call__(self, batch_size=1, batch_size_eval=1, eval_mode='dev', use_collate=False):
        if not use_collate:
            ...
            # dpo特殊处理对齐
            if (self.loader_name == 'LLM_DPO' or self.loader_name == 'LLM_DPO_EN'):
