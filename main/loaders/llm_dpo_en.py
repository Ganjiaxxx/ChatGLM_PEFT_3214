...
class LLMDPOENDataset(Dataset):
    ...
    def apply_template_with_char_tokenize(self, messages: List[dict], content_role="assistant", add_generation_prompt=False):
        ...
        # 先进行分割成列表，英文不是一个字符字符编码，而是一个词
        split_content = self.split_any(orig_content)
        char_ids = []
        # 不能使用列表推导式，否则char_ids会是嵌套的list，且英文可能存在一个词对应多个Token
        for ch in split_content:
            # 避免对空字符进行编码（虽然通常不会有）
            if ch == "":
                continue
    ...
    def build_single_message_dpo(self, t):
        ...
        if self.config.model_type == 'llama':
            ...
            if "<|begin_of_text|>" not in self.tokenizer.special_tokens_map.values():
                tokens_to_train = []
                # 先进行分割成列表，英文不是一个字符字符编码，而是一个词
                split_content = self.split_any(t['content'])
                # 不能使用列表推导式，否则tokens_to_train会是嵌套的list，且英文可能存在一个词对应多个Token
    ...
    def split_any(self, text: str) -> list[str]:
        """
        将字符串按以下规则切分：
        - 遇到 '('，把 '(' 作为一个 token，然后将后续直到 ')' 之间的每个字符单独拆成 token，最后把 ')' 也作为一个 token。
        - 在括号外，将所有连续的字符（只要不是 '('）一次性收为一个整体 token。
        """
        tokens = []
        i = 0
        n = len(text)
        while i < n:
            ...
            if ch == '(':
                ...
                while j < n and text[j] != '(':
                    j += 1
                # text[i:j] 可能是任意字符的组合，直接作为一个 token
    ...
    def process_item(self, item):
        ...
        #chosen_full_tokens = [self.tokenizer.convert_tokens_to_ids('<|begin_of_text|>')]
        ...
        #rejected_full_tokens = [self.tokenizer.convert_tokens_to_ids('<|begin_of_text|>')]
        ...
        '''role = t['role']
            ...
            if (role == "user"):
                ids = self.tokenizer.apply_chat_template([t])
                prompt = ids
            # assistant部分每个词单独进行编码
            elif (role == "assistant"):
                # 对应[gMASK] <sop> <|assistant|> \n
                ids = [151331, 151333, 151337, 198]
                # 先进行分割成列表，英文不是一个字符字符编码，而是一个词
                split_content = self.split_any(t['content'])
                # 不能使用列表推导式，否则ids会是嵌套的list，且英文可能存在一个词对应多个Token
                for char in split_content:
                    token_ids = self.tokenizer.encode(char, add_special_tokens=False)
        ...
        # 先进行分割成列表，英文不是一个字符字符编码，而是一个词
        split_content1 = self.split_any(chosen)
        # 不能使用列表推导式，否则ids会是嵌套的list，且英文可能存在一个词对应多个Token
        for char in split_content1:
            token_ids = self.tokenizer.encode(char, add_special_tokens=False)
            ids1.extend(token_ids)
        chosen_full_tokens = []
        chosen_full_tokens.extend(prompt)
        chosen_full_tokens.extend(ids1)
        ...
        # 先进行分割成列表，英文不是一个字符字符编码，而是一个词
        split_content2 = self.split_any(rejected)
        # 不能使用列表推导式，否则ids会是嵌套的list，且英文可能存在一个词对应多个Token
        for char in split_content2:
            token_ids = self.tokenizer.encode(char, add_special_tokens=False)
            ids2.extend(token_ids)
    ...
    def __getitem__(self, index):
        ...
        prompt, chosen, rejected, chosen_mask, rejected_mask, input_ids, labels = self.process_item(data).values()
